...
const Card = (props) => {
  ...
  return (
    <div className="rounded-xl p-4 bg-white duration-300 hover:shadow-xl active:shadow-lg">
      <img className="w-full" src={course.thumbnail} alt={course.title} />
      <p className="text-dark1 text-base font-semibold mt-4 mb-3">
        {course.title}
      </p>
      <p className="text-dark2 text-sm font-normal mb-5 md:h-16 text-clip">
        {course.description}
      </p>
      <div className="flex justify-between items-center mb-7 mt-5">
        <div className="flex gap-3 items-center">
          <img className="w-6 h-6" src={dollarLogo} alt="doller-logo" />
          <p className="text-dark2 text-base font-medium">Price: {course.price}</p>
        </div>
        <div className="flex gap-3 items-center">
          <img className="w-6 h-6" src={bookLogo} alt="book-logo" />
          <p className="text-dark2 text-base font-medium">Credit: {course.credit}hr</p>
        </div>
      </div>
      <button
